...
public class FileUtils {
  ...
  //默认编码 utf-8
  private static String mDefaultCharsetName = "utf-8";

  /**
   * 判断SD卡是否可用
   *
   * @return true(可用) fals（不可用）
   */
  ...
  public static String readFileToString(String filePath, String charsetName) {
    ...
    if (TextUtils.isEmpty(charsetName)) {
      charsetName = mDefaultCharsetName;
    }
    ...
    try {
      ...
      Log.e("readFileToString", "Exception = " + e.toString());
      e.printStackTrace();
      return null;
    } finally {
      if (reader != null) {
        try {
          ...
          e.printStackTrace();
          Log.e("readFileToString   ", "IOException = " + e.toString());
        }
      }
    }
  }
  ...
  /**
   * 读取文件内容，输出List字符串集合 (默认utf-8编码)
   *
   * @param filePath 文件目录
   * @return 文件不存在返回null，否则返回字符串集合
   */
  public static List<String> readFileToList(String filePath) {
    return readFileToList(filePath, mDefaultCharsetName);
  }
   ...
   * @return 文件不存在返回null，否则返回字符串集合
   */
  private static List<String> readFileToList(String filePath, String charsetName) {
    if (TextUtils.isEmpty(filePath)) {
      return null;
    }
    if (TextUtils.isEmpty(charsetName)) {
    ...
    try {
      ...
      Log.e("readFileToString Exception", "Exception = " + e.toString());
      e.printStackTrace();
      return null;
    } finally {
      if (reader != null) {
        try {
          ...
          e.printStackTrace();
          Log.e("readFileToString  finally IOException", "IOException = " + e.toString());
        }
      }
    }
  }
   ...
   */
  public static boolean writeFileFromStr(String filePath, String content, boolean append) {
    if (TextUtils.isEmpty(filePath)) {
      return false;
    }
    ...
    try {
      ...
      e.printStackTrace();
      Log.e("writeFileFromStr IOException", "IOException = " + e.toString());
      return false;
    } finally {
      if (fileWriter != null) {
        try {
          ...
          e.printStackTrace();
          Log.e("writeFileFromStr  finally IOException", "IOException = " + e.toString());
        }
      }
    }
  }
   ...
   */
  public static boolean writeFileFromIs(String filePath, InputStream stream) {
    return writeFileFromIs(filePath, stream, false);
  }
   ...
   * @return 写入成功返回true，否则返回false
   */
  public static boolean writeFileFromIs(String filePath, InputStream stream, boolean append) {
    if (TextUtils.isEmpty(filePath)) {
    ...
    return writeFileFromIs(new File(filePath), stream, append);
  }
   ...
   * @return 写入成功返回true，否则返回false
   */
  public static boolean writeFileFromIs(File file, InputStream stream, boolean append) {
    if (file == null) {
    ...
    try {
      ...
      e.printStackTrace();
      Log.e("writeFileFromIs IOException", "writeFileFromIs = " + e.toString());
      return false;
    } finally {
      if (out != null) {
        try {
   ...
   */
  public static boolean writeFileFromIs(File file, InputStream stream) throws IOException {
  ...
  public static boolean copyFile(String sourceFilePath, String destFilePath) throws IOException {
    ...
    return writeFileFromIs(destFilePath, inputStream);
  }
  ...
  public static boolean createFile(File file) {
    String floadPath = getFloadPath(file.getAbsolutePath());
    if (null == file || !makeDirs(floadPath)) {
      return false;
    }
    if (!file.exists()) {
      try {
        ...
        e.printStackTrace();
        Log.e("createFile IOException", "createFile = " + e.toString());
        return false;
      }
    }
    return false;
   ...
   */
  private static String getFloadPath(String filePath) {
    if (TextUtils.isEmpty(filePath)) {
      return filePath;
    }
    int filePos = filePath.lastIndexOf(File.separator);
    String path = (filePos == -1) ? "" : filePath.substring(0, filePos);
    return path;
  }
