 ...
 * warn： 切记，此类须在setContentView()后调用
 */
public class CustomStatusBar {
   ...
   * @param statusBar 状态栏的背景色需要设置和status一致(可以为空，为空时默认整个布局的背景色)
   */
  public static void setTranslucentForDrawLayout(Activity activity, DrawerLayout drawerLayout, boolean isWhiteBg,
      int statusBarAlpha, ViewGroup statusBar) {
   ...
   * @param isAll true===导航栏和状态栏都变色，false === 仅状态栏变色
   */
  private static void setTranslucent(Activity activity, DrawerLayout drawerLayout, int statusBarAlpha, boolean diff,
      ViewGroup statusBar, boolean isAll) {
   ...
   * @param statusBar 状态栏的背景色需要设置和status一致(可以为空，为空时默认整个布局的背景色)
   */
  private static void setTransparent(Activity activity, DrawerLayout drawerLayout, boolean diff, boolean isAll,
      ViewGroup statusBar) {
    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
      return;
    }
  ...
  private static void setSystemUi(Activity activity) {
    activity.getWindow()
        .getDecorView()
        .setSystemUiVisibility(SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN | SYSTEM_UI_FLAG_LIGHT_STATUS_BAR);
  }
   ...
   * @param drawerLayout 是否使用了drawerLayout布局，可以为空，即非drawerLayout布局
   */
  @TargetApi(Build.VERSION_CODES.ICE_CREAM_SANDWICH) private static void setRootView(Activity activity, boolean diff,
      ViewGroup statusBar, DrawerLayout drawerLayout) {
